...
def njetWeights(filename):
    ...
    cuts_DYElec = ["elZinv", "elZinv_0b", "elZinv_g1b"]
    cuts_TT = ["elmuZinv", "elmuZinv_0b", "elmuZinv_g1b"]
    selection = "ht200_dphi"#"loose0"
    # histo names
    ...
    hname1Elec = "cntNJetsPt30Eta24Zinv/DataMC_DoubleEG_nj_%(cut)s_%(selection)scntNJetsPt30Eta24ZinvcntNJetsPt30Eta24ZinvDatadata"
    hnames2Elec = ["cntNJetsPt30Eta24Zinv/DataMC_DoubleEG_nj_%(cut)s_%(selection)scntNJetsPt30Eta24ZinvcntNJetsPt30Eta24ZinvDYstack",###electrons######
               "cntNJetsPt30Eta24Zinv/DataMC_DoubleEG_nj_%(cut)s_%(selection)scntNJetsPt30Eta24ZinvcntNJetsPt30Eta24ZinvDY HT<100stack",
               "cntNJetsPt30Eta24Zinv/DataMC_DoubleEG_nj_%(cut)s_%(selection)scntNJetsPt30Eta24ZinvcntNJetsPt30Eta24Zinvt#bar{t}stack",
               "cntNJetsPt30Eta24Zinv/DataMC_DoubleEG_nj_%(cut)s_%(selection)scntNJetsPt30Eta24ZinvcntNJetsPt30Eta24Zinvsingle topstack",
               "cntNJetsPt30Eta24Zinv/DataMC_DoubleEG_nj_%(cut)s_%(selection)scntNJetsPt30Eta24ZinvcntNJetsPt30Eta24Zinvt#bar{t}Zstack",
               "cntNJetsPt30Eta24Zinv/DataMC_DoubleEG_nj_%(cut)s_%(selection)scntNJetsPt30Eta24ZinvcntNJetsPt30Eta24ZinvDibosonstack",
               "cntNJetsPt30Eta24Zinv/DataMC_DoubleEG_nj_%(cut)s_%(selection)scntNJetsPt30Eta24ZinvcntNJetsPt30Eta24ZinvRarestack"]
    ...
    for cut in cuts_TT:
        ...
        SFs["TT_%s"%(cut)] = newh ###commented out believing the muon result would be the best
        newh.Write()
    ...
    for cut in cuts_DY:
        ...
        print 'reweighting',h2s[2]
        h2s[2] = reweight(h2s[2], SFs["TT_%s"%(cut.replace("mu","elmu"))])
        print 'about to subtract',h2s[2:],'from',h1, 'which was named',hname1_DY
        data_subtracted = subtract(h1, h2s[2:])
        newname = "DataMC_nj_%s_%s"%(cut,selection)
        newh = makeRatio(data_subtracted, h2s[:1], bins, newname)
        newh.Write()

    for cut in cuts_DYElec:
        print '==='*100
        hname1_DY = hname1Elec % {"cut":cut, "selection":selection}
        hnames2_DY = [elem % {"cut":cut, "selection":selection} for elem in hnames2Elec]   
        # Get all histos
        h1 = f.Get(hname1_DY)
        h2s = [f.Get(hname2_DY) for hname2_DY in hnames2_DY]
        # apply weights to ttbar
        h2s[2] = reweight(h2s[2], SFs["TT_%s"%(cut.replace("el","elmu"))])
        print 'about to subtract',h2s[2:],'from',h1, 'which was named',hname1_DY
        data_subtracted = subtract(h1, h2s[2:])
        newname = "DataMC_nj_%s_%s"%(cut,selection)
        newh = makeRatio(data_subtracted, h2s[:1], bins, newname)
...
def normWeight(filename):
    ...
    for cut in cuts_DY:
        ...
        data_subtracted = subtract(h1, h2s[2:])
        #print 'once again, subtracting',h2s[2:],'from',h1
    ...
    selection2 = "blnotag"
    # histo names
    hname1 = "cntCSVSZinv/DataMC_DoubleEG_nb_%(cut)s_%(selection)scntCSVSZinvcntCSVSZinvDatadata"
    hnames2 = ["cntCSVSZinv/DataMCw_DoubleEG_nb_%(cut)s_%(selection)scntCSVSZinvcntCSVSZinvDYstack",
               "cntCSVSZinv/DataMCw_DoubleEG_nb_%(cut)s_%(selection)scntCSVSZinvcntCSVSZinvDY HT<100stack",
               "cntCSVSZinv/DataMCw_DoubleEG_nb_%(cut)s_%(selection)scntCSVSZinvcntCSVSZinvt#bar{t}stack",
               "cntCSVSZinv/DataMCw_DoubleEG_nb_%(cut)s_%(selection)scntCSVSZinvcntCSVSZinvsingle topstack",
               "cntCSVSZinv/DataMCw_DoubleEG_nb_%(cut)s_%(selection)scntCSVSZinvcntCSVSZinvt#bar{t}Zstack",
               "cntCSVSZinv/DataMCw_DoubleEG_nb_%(cut)s_%(selection)scntCSVSZinvcntCSVSZinvDibosonstack",
               "cntCSVSZinv/DataMCw_DoubleEG_nb_%(cut)s_%(selection)scntCSVSZinvcntCSVSZinvRarestack"
               ]
    # Procedure: 1. Grab the njet reweighted MC
    #            2. Subtract non-DY MC from data
    #            3. Make ratio of subtracted data and DY
...
def shapeSyst(filename):
    ...
    #commence electrons
    hnameData = "%(var)s/DataMCw_DoubleEG_%(name)s_elZinv_loose0_mt2%(var)s%(var)sDatadata"
    hnames2 =  ["%(var)s/DataMCw_DoubleEG_%(name)s_elZinv_loose0_mt2%(var)s%(var)sDYstack",
                "%(var)s/DataMCw_DoubleEG_%(name)s_elZinv_loose0_mt2%(var)s%(var)sDY HT<100stack",
                "%(var)s/DataMCw_DoubleEG_%(name)s_elZinv_loose0_mt2%(var)s%(var)st#bar{t}stack",
                "%(var)s/DataMCw_DoubleEG_%(name)s_elZinv_loose0_mt2%(var)s%(var)ssingle topstack",
                "%(var)s/DataMCw_DoubleEG_%(name)s_elZinv_loose0_mt2%(var)s%(var)st#bar{t}Zstack",
                "%(var)s/DataMCw_DoubleEG_%(name)s_elZinv_loose0_mt2%(var)s%(var)sDibosonstack",
                "%(var)s/DataMCw_DoubleEG_%(name)s_elZinv_loose0_mt2%(var)s%(var)sRarestack"]
    ...
    for var in varList:
        ...
        hData = f.Get(hnameData%{"name":var[0], "var":var[1]})
        h2s   = [f.Get(hname2%{"name":var[0], "var":var[1]}) for hname2 in hnames2]
        print 'hnameData',hnameData
        print 'hnameData%{"name":var[0], "var":var[1]}=',hnameData%{"name":var[0], "var":var[1]},
        #if 'DataMCw_DoubleEG_nt' in hnameData%{"name":var[0], "var":var[1]}: continue
        # subtract relevant histograms from data
        data_subtracted = subtract(hData, h2s[2:])

        newname = "ShapeRatioElec_%s"%var[0]
        newh = makeRatio(data_subtracted, h2s[:1], newname=newname, bins=var[2])
        ...
        #newh.DrawCopy()
        #c.Print("shapeSyst_%s.png"%var[0])
        c.Print("shapeSystElec_%s.pdf"%var[0])
        print 'writing electron'
        fout.cd()
        newh.Write()
...
def systHarvest(filename):
    ...
    # b-tag uncertanties
    hBTagNom = f4.Get("nSearchBin/BTagUncert_nSearchBinnSearchBinnSearchBinZ#rightarrow#nu#nu btag weight Centralsingle")
    hBTagUp =  f4.Get("nSearchBin/BTagUncert_nSearchBinnSearchBinnSearchBinZ#rightarrow#nu#nu btag weight Upsingle")####Trigger was btag
    hBTagDn =  f4.Get("nSearchBin/BTagUncert_nSearchBinnSearchBinnSearchBinZ#rightarrow#nu#nu btag weight Downsingle")####Trigger was btag

    hBMistagNom = f4.Get("nSearchBin/BMistagUncert_nSearchBinnSearchBinnSearchBinZ#rightarrow#nu#nu bmistag weight Centralsingle")
    ...
    # Get shape central value uncertainty
    #f = TFile("systematics.root")
    hShape_MET_Nom = f.Get("nSearchBin/systWgtMETElec_cleanMetPtnSearchBinnSearchBinNominalsingle")
    hShape_MET_Var = f.Get("nSearchBin/systWgtMETElec_cleanMetPtnSearchBinnSearchBinvariedsingle")
    hShape_MT2_Nom = f.Get("nSearchBin/systWgtMT2Elec_best_had_brJet_MT2ZinvnSearchBinnSearchBinNominalsingle")
    hShape_MT2_Var = f.Get("nSearchBin/systWgtMT2Elec_best_had_brJet_MT2ZinvnSearchBinnSearchBinvariedsingle")
    hShape_NT_Nom  = f.Get("nSearchBin/systWgtNTElec_nTopCandSortedCntZinvnSearchBinnSearchBinNominalsingle")
    hShape_NT_Var  = f.Get("nSearchBin/systWgtNTElec_nTopCandSortedCntZinvnSearchBinnSearchBinvariedsingle")
    hShape_NB_Nom  = f.Get("nSearchBin/systWgtNBElec_cntCSVSZinvnSearchBinnSearchBinNominalsingle")
    hShape_NB_Var  = f.Get("nSearchBin/systWgtNBElec_cntCSVSZinvnSearchBinnSearchBinvariedsingle")
    ...
    hShape_final = hShape_MET_ratio.Clone("shape_centralElec")
    ...
    #fout = TFile("syst_all.root", "RECREATE")#don't need this the second time
    hShape_final.Write()


    # Get shape stats uncertainty
    ###don't need this for electrons###f2 = TFile("syst_nJetWgt.root")
    hShapeStat = f2.Get("syst68MaxElec").Clone("shape_statElec")

    hAvgWgt = f2.Get("avgWgtElec").Clone("avgWgtElec")
    hNEff   = f2.Get("neffElec").Clone("neffElec")

    fout.cd()
    hShapeStat.Write()

    '''
    #???????Get PDF and scale uncertainties##### anything special for electrons here?????????????????
    f3 = TFile("syst_scalePDF.root")
    hScaleUp = f3.Get("nSearchBin_ratio_scale_up").Clone("scale_up")
    hScaleDn = f3.Get("nSearchBin_ratio_scale_down").Clone("scale_down")
    hPDFUp = f3.Get("nSearchBin_ratio_pdf_up").Clone("pdf_up")
    ...
    # Get central, MC stats, closure, trigger, MEU, and JEU
    hMC = f4.Get("nSearchBin/NJetWgt_nSearchBinElecnSearchBinnSearchBinZ#rightarrow#nu#nusingle")
    hMCstats = hMC.Clone("MC_statsElec")
    for i in xrange(1, hMCstats.GetNbinsX() + 1):
        if hMC.GetBinContent(i) > 0.00000001:
            hMCstats.SetBinContent(i, hMC.GetBinError(i) / hMC.GetBinContent(i))
        else:
    ...
    hClosureZ = f4.Get("nSearchBin/nSearchBinnSearchBinnSearchBinZ#rightarrow#nu#nusingle")
    hClosureDY = f4.Get("nSearchBin/elec_nSearchBinnSearchBinnSearchBinDY#rightarrowee no e, Z eff+accsingle")
    hClosureRatio = hClosureZ.Clone("MC_closureElec")
    hClosureRatio.Add(hClosureDY, -1)
    hClosureRatio.Divide(hClosureZ)
    for i in xrange(1, hClosureRatio.GetNbinsX() + 1):
        hClosureRatio.SetBinContent(i, abs(hClosureRatio.GetBinContent(i)))
    ...
    hJECNom = f4.Get("nSearchBin/syst_JESUncert_nSearchBinElecnSearchBinnSearchBinZ#rightarrow#nu#nu Njet+norm weightsingle")
    hJECUp = f4.Get("nSearchBinJEUUp/syst_JESUncert_nSearchBinElecnSearchBinJEUUpnSearchBinJEUUpZ#rightarrow#nu#nu JEC Upsingle")
    hJECDn = f4.Get("nSearchBinJEUDn/syst_JESUncert_nSearchBinElecnSearchBinJEUDnnSearchBinJEUDnZ#rightarrow#nu#nu JEC Downsingle")

    hMECNom = f4.Get("nSearchBin/syst_MESUncert_nSearchBinElecnSearchBinnSearchBinZ#rightarrow#nu#nu Njet+norm weightsingle")
    hMECUp = f4.Get("nSearchBinMEUUp/syst_MESUncert_nSearchBinElecnSearchBinMEUUpnSearchBinMEUUpZ#rightarrow#nu#nu MEC Upsingle")
    hMECDn = f4.Get("nSearchBinMEUDn/syst_MESUncert_nSearchBinElecnSearchBinMEUDnnSearchBinMEUDnZ#rightarrow#nu#nu MEC Downsingle")

    hJECUp_ratio = hJECUp.Clone("jeu_upElec")
    hJECUp_ratio.Add(hJECNom, -1.0)
    hJECUp_ratio.Divide(hJECNom)

    hJECDn_ratio = hJECDn.Clone("jeu_downElec")
    hJECDn_ratio.Add(hJECNom, -1.0)
    hJECDn_ratio.Divide(hJECNom)

    hMECUp_ratio = hMECUp.Clone("meu_upElec")
    hMECUp_ratio.Add(hMECNom, -1.0)
    hMECUp_ratio.Divide(hMECNom)

    hMECDn_ratio = hMECDn.Clone("meu_downElec")
    hMECDn_ratio.Add(hMECNom, -1.0)
    hMECDn_ratio.Divide(hMECNom)
    ...
    hTrigNom = f4.Get("nSearchBin/TriggerWgt_nSearchBinElecnSearchBinnSearchBinZ#rightarrow#nu#nu Trigger weight Centralsingle")
    hTrigUp =  f4.Get("nSearchBin/TriggerWgt_nSearchBinElecnSearchBinnSearchBinZ#rightarrow#nu#nu Trigger weight Upsingle")
    hTrigDn =  f4.Get("nSearchBin/TriggerWgt_nSearchBinElecnSearchBinnSearchBinZ#rightarrow#nu#nu Trigger weight Downsingle")
    ...
    # b-tag uncertanties
    hBTagNom = f4.Get("nSearchBin/BTagUncert_nSearchBinElecnSearchBinnSearchBinZ#rightarrow#nu#nu btag weight Centralsingle")
    hBTagUp =  f4.Get("nSearchBin/BTagUncert_nSearchBinElecnSearchBinnSearchBinZ#rightarrow#nu#nu btag weight Upsingle")
    hBTagDn =  f4.Get("nSearchBin/BTagUncert_nSearchBinElecnSearchBinnSearchBinZ#rightarrow#nu#nu btag weight Downsingle")

    hBMistagNom = f4.Get("nSearchBin/BMistagUncert_nSearchBinElecnSearchBinnSearchBinZ#rightarrow#nu#nu bmistag weight Centralsingle")#bmistag!
    hBMistagUp =  f4.Get("nSearchBin/BMistagUncert_nSearchBinElecnSearchBinnSearchBinZ#rightarrow#nu#nu bmistag weight Upsingle")#bmistag!
    hBMistagDn =  f4.Get("nSearchBin/BMistagUncert_nSearchBinElecnSearchBinnSearchBinZ#rightarrow#nu#nu bmistag weight Downsingle")#change to Down#bmistag!

    hBTagUp_Ratio = hBTagUp.Clone("btag_upElec")
    hBTagUp_Ratio.Add(hBTagNom, -1.0)
    hBTagUp_Ratio.Divide(hBTagNom)

    hBTagDn_Ratio = hBTagDn.Clone("btag_dnElec")
    hBTagDn_Ratio.Add(hBTagNom, -1.0)
    hBTagDn_Ratio.Divide(hBTagNom)

    hBMistagUp_Ratio = hBMistagUp.Clone("bmistag_upElec")
    hBMistagUp_Ratio.Add(hBMistagNom, -1.0)
    hBMistagUp_Ratio.Divide(hBMistagNom)

    hBMistagDn_Ratio = hBMistagDn.Clone("bmistag_dnElec")
    hBMistagDn_Ratio.Add(hBMistagNom, -1.0)
    hBMistagDn_Ratio.Divide(hBMistagNom)
    ...
    # calculate stupid pull histo here
    hZnunu = f4.Get("nSearchBin/nSearchBinnSearchBinnSearchBinZ#rightarrow#nu#nusingle")
    hDYll  = f4.Get("nSearchBin/elec_nSearchBinnSearchBinnSearchBinDY#rightarrowee no e, Z eff+accsingle")

    hMCPull = TH1D("MCpullElec", "MCpullElec", 15, -5, 5)
    hMCPull.Sumw2()
    hMCPull2 = hZnunu.Clone("MCPull2Elec")
    for i in xrange(1, hZnunu.GetNbinsX() + 1):
        try: pull = (hZnunu.GetBinContent(i) - hDYll.GetBinContent(i)) / math.sqrt(hZnunu.GetBinError(i)**2 + hDYll.GetBinError(i)**2)
        except: pull = 0
        hMCPull.Fill(pull)
        hMCPull2.SetBinContent(i, pull)
    ...
    #Central value
    hPrediction = f4.Get("nSearchBin/TriggerWgt_nSearchBinElecnSearchBinnSearchBinZ#rightarrow#nu#nu Njet+norm weightsingle").Clone("central_predictionElec")
    fout.cd()
    hPrediction.Write()

    # make proto data card

    hJEC_ratio_sym = hJECUp_ratio.Clone("hJEC_ratio_symElec")
    hMEC_ratio_sym = hMECUp_ratio.Clone("hMEC_ratio_symElec")
    hScale_sym     = hScaleUp.Clone("hScale_symElec")
    hPDF_sym       = hPDFUp.Clone("hPDF_symElec")
    hTrig_sym      = hTrigUp_ratio.Clone("hTrig_symElec")
    hBTag_sym      = hBTagUp_Ratio.Clone("hBTag_symElec")
    hBMistag_sym   = hBMistagUp_Ratio.Clone("hBMistag_symElec")
    ...
    hOther = hJEC_ratio_sym.Clone("hOtherElec")
    for i in xrange(1, hOther.GetNbinsX() + 1):
        hOther.SetBinContent(i, sqrt(hJEC_ratio_sym.GetBinContent(i)**2 + hMEC_ratio_sym.GetBinContent(i)**2 + hScale_sym.GetBinContent(i)**2 + hPDF_sym.GetBinContent(i)**2 + hTrig_sym.GetBinContent(i)**2 ++ hBTag_sym.GetBinContent(i)**2 + hBMistag_sym.GetBinContent(i)**2))
    hOther.Write()

    hists = [("syst_unc_shape_central_upElec",   hShape_final),
             ("syst_unc_shape_central_dnElec",   hShape_final),
             ("syst_unc_shape_stat_upElec",      hShapeStat),
             ("syst_unc_shape_stat_dnElec",      hShapeStat),
             #("stat_unc_upElec",                 hMCstats),
             #("stat_unc_dnElec",                 hMCstats),
             ("syst_unc_jeu_upElec",             hJEC_ratio_sym),
             ("syst_unc_jeu_dnElec",             hJEC_ratio_sym),
             ("syst_unc_meu_upElec",             hMEC_ratio_sym),
             ("syst_unc_meu_dnElec",             hMEC_ratio_sym),
             ("syst_unc_scale_upElec",           hScale_sym),
             ("syst_unc_scale_dnElec",           hScale_sym),
             ("syst_unc_pdf_upElec",             hPDF_sym),
             ("syst_unc_pdf_dnElec",             hPDF_sym),
             ("syst_unc_trig_upElec",            hTrig_sym),
             ("syst_unc_trig_dnElec",            hTrig_sym),
             ("syst_unc_btag_upElec",            hBTagUp_Ratio),
             ("syst_unc_btag_dnElec",            hBTagDn_Ratio),
             ("syst_unc_bmistag_upElec",         hBMistagUp_Ratio),
             ("syst_unc_bmistag_dnElec",         hBMistagDn_Ratio),
             ]

    print '=='*10,'Electrons','='*10
    print "luminosity = 2262"
    print "channels =", NSB
    print "sample = zinv"
    print ""
    ...
    print "%-25s = %s"%("syst_unc_norm_up", ' '.join(NSB*["%8.5f" % 0.18035]))
    print "%-25s = %s"%("syst_unc_norm_dn", ' '.join(NSB*["%8.5f" % 0.18035]))
